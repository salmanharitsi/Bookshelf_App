var namaPengguna = localStorage.getItem('namaPengguna');

if (namaPengguna) {
  document.getElementById('greeting').textContent = 'Halo, ' + namaPengguna + '!';
} else {
...
const makeBook = (title, author, year, isCompleted) => {
    ...
    if(isCompleted) {
        image.setAttribute('src', '../asset/unread.jpg')
      } else {
        image.setAttribute('src', '../asset/unread.jpg')
      }
...
const createReadButton = () => {
    return createButton("read-button", "sudah dibaca", function (event) {
...
const createTrashButton = () => {
    return createButton("trash-book", "hapus buku", function(event){
